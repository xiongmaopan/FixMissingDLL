...
// 合并手动和自动生成的指南 + 定时发布指南
// 重要：只包含今天及之前发布的内容，防止未来日期的文章提前发布
const today = new Date().toISOString().split('T')[0]; // 格式: YYYY-MM-DD
const scheduledGuides = (scheduledGuidesData as any).guides || [];

// 过滤：只保留 publishDate <= 今天的内容
const publishedScheduledGuides = (scheduledGuides as Guide[]).filter(
  guide => guide.publishDate <= today
);
...
const mergedGuides: Guide[] = [
  ...
  ...publishedScheduledGuides.filter(
